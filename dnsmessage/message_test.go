...
// TestName tests encoding and decoding of names that get decoded in their
// original format.
func TestName(t *testing.T) {
	names := []string{
		...
		// Names with escaped characters that are special in RFC 1035 section
		// 5.1 master files.
		"\\\t.example.",
		`\ .example.`,
		`\$.example.`,
		...
		// Names with decimal-escaped characters that are special in RFC 1035
		// section 5.1 master files.
		`\010.example.`,
		`\013.example.`,
	...
	for _, name := range names {
		...
		if got := n.String(); got != name {
			t.Errorf("got %#v.String() = %q, want = %q", n, got, name)
			continue
		}
	}
}

// TestNameTransform tests encoding and decoding of names that get decoded in a
...
func TestNameTransform(t *testing.T) {
	tests := []struct {
		...
		// Names with unescaped characters that are special in RFC 1035 section
		//  5.1 master files.
		{"a$.example.", `a\$.example.`},
		{"a.$.example.", `a.\$.example.`},
		{"@.example.", `\@.example.`},
		...
		{`\032.example.`, "\\ .example."},
		{`\036.example.`, "\\$.example."},
		{`a\036.example.`, "a\\$.example."},
		{`a.\036.example.`, "a.\\$.example."},
		{`\046.example.`, "\\..example."},
		{`\034.example.`, "\\\".example."},
		{`a\034.example.`, "a\\\".example."},
		{`\064.example.`, "\\@.example."},
		{`\040.example.`, "\\(.example."},
		{`a\040.example.`, "a\\(.example."},
		{`\041.example.`, "\\).example."},
		{`a\041.example.`, "a\\).example."},
		{`\059.example.`, "\\;.example."},
		{`a\059.example.`, "a\\;.example."},

		// Names with other decimal-escaped non-LDH octets.
		{`\035.example.`, "#.example."},
		{`\042.example.`, "*.example."},
		{`\047\042.example.`, "/*.example."},
		{`\065\042.example.`, "A*.example."},
		{`a.\042.example.`, "a.*.example."},
		{`\042.\042.example.`, "*.*.example."},
		{`\126.example.`, "~.example."},
	}

	for _, test := range tests {
		n, err := NewName(test.in)
		if err != nil {
			t.Errorf("NewName(%q) = %v", test, err)
			continue
		}
		if ns := n.String(); ns != test.want {
			t.Errorf("got %#v.String() = %q, want = %q", n, ns, test.want)
			continue
		}
	}
}

// TestNameError tests encoding invalid names.
func TestNameError(t *testing.T) {
	tests := []struct {
		...
		{"com", errNonCanonicalName},
		{"google.com", errNonCanonicalName},
		{"google..com.", errZeroSegLen},
		{".google.com.", errZeroSegLen},
		{"google.com..", errZeroSegLen},
		{"www..google.com.", errZeroSegLen},
		{"a.b..com.", errZeroSegLen},
		{"a.b..", errZeroSegLen},

		// Names with escaped characters that are special in RFC 1035 section
		// 5.1 master files.
		{"\\\n.example.", errInvalidEscape},
		{"\\\r.example.", errInvalidEscape},

		// Names with other escaped non-LDH octets.
		{"\\\x00.example.", errInvalidEscape},
		{"\\\x01.example.", errInvalidEscape},
		{"\\\x1F.example.", errInvalidEscape},
		{"\\\x7F.example.", errInvalidEscape},
		{"\\\x80.example.", errInvalidEscape},
		{"\\\xC0.example.", errInvalidEscape},
		{"\\\xE0.example.", errInvalidEscape},
		{"\\\xF0.example.", errInvalidEscape},
		{"\\\xF8.example.", errInvalidEscape},
		{"\\\xFC.example.", errInvalidEscape},
		{"\\\xFE.example.", errInvalidEscape},
		{"\\\xFF.example.", errInvalidEscape},

		// Names with bad escapes.
		{`example\.`, errNonCanonicalName},
		{`example\0.`, errCalcLen},
		{`example\00.`, errInvalidEscape},
		{`\0.example.`, errInvalidEscape},
		{`\00.example.`, errInvalidEscape},
		{`\0  .example.`, errInvalidEscape},
		{`\00a.example.`, errInvalidEscape},
		{`\7up.example.`, errInvalidEscape},
		{`\256.example.`, errInvalidEscape},

		// Names which are too long.
		{"123456789a123456789b123456789c123456789d123456789e123456789f1234.", errSegTooLong},
	}

	for _, test := range tests {
		if _, err := NewName(test.in); err != test.err {
			t.Errorf("got NewName(%q) = _, %v; want = _, %v", test.in, err, test.err)
		}
	}
}
...
func TestNamePackUnpack(t *testing.T) {
	...
	for _, compression := range []bool{false, true} {
		for _, test := range tests {
			...
			in, err := NewName(test.in)
			if err != nil {
				t.Errorf("NewName(%q) = %v", test.in, err)
				continue
			}
			b, err := in.pack(nil, comp, 0)
			if err != nil {
				t.Errorf("%q.pack(/* compression = %t */) = %v", test.in, compression, err)
				continue
			}
			var out Name
			if _, err := out.unpack(b, 0); err != nil {
				t.Errorf("compression = %t: Name.unpack(%q) = %v", compression, test.in, err)
				continue
			}
			if got := out.String(); got != test.want {
				t.Errorf("compression = %t: got %q.String() = %q, want = %q", compression, test.in, got, test.want)
			}
		}
	}
}
...
func TestDNSPackUnpack(t *testing.T) {
	wants := []Message{
		...
		},
		{
			Questions: []Question{
				{
					Name:  MustNewName("foo.bar."),
					Type:  TypeAAAA,
					Class: ClassINET,
				},
				{
					Name:  MustNewName("foo.baz."),
					Type:  TypeAAAA,
					Class: ClassINET,
				},
				{
					Name:  MustNewName(`foo\003baz.`),
					Type:  TypeAAAA,
					Class: ClassINET,
				},
			},
...
const testName = "foo.bar.example.com."

func TestVeryLongTxt(t *testing.T) {
	want := Resource{
		ResourceHeader{
			Name:  MustNewName(testName),
			Type:  TypeTXT,
			Class: ClassINET,
		},
		&TXTResource{[]string{
...
func BenchmarkNameEncoding(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		if _, err := NewName(testName); err != nil {
			b.Errorf("NewName(%q) = %v", testName, err)
		}
	}
}

func TestNameEncodingAllocs(t *testing.T) {
	if runtime.Compiler != "gc" {
		t.Skipf("Test depends on gc implementation details. Test compiled with %s.", runtime.Compiler)
	}
	if allocs := testing.AllocsPerRun(100, func() {
		if _, err := NewName(testName); err != nil {
			t.Errorf("NewName(%q) = %v", testName, err)
		}
	}); allocs > 0.5 {
		t.Errorf("got testing.AllocsPerRun() = %f, want ~0", allocs)
	}
}

func BenchmarkNameDecoding(b *testing.B) {
	name := MustNewName(testName)
	buf := make([]byte, nameLen)
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		if _, err := name.Bytes(buf[:0]); err != nil {
			b.Errorf("%q.Bytes() = %v", testName, err)
		}
	}
}

func TestNameDecodingAllocs(t *testing.T) {
	name := MustNewName(testName)
	buf := make([]byte, nameLen)
	if allocs := testing.AllocsPerRun(100, func() {
		if _, err := name.Bytes(buf[:0]); err != nil {
			t.Errorf("%q.Bytes() = %v", testName, err)
		}
	}); allocs > 0.5 {
		t.Errorf("got testing.AllocsPerRun() = %f, want ~0", allocs)
	}
